...
Catatan metodologis: setiap kondisi menerapkan \textit{equivalent conditioning level}, bukan prompt string yang identik. DeepBach dikontrol melalui tensor constraint, Coconet melalui injeksi seed nada, NotaGen melalui metadata teks --- sesuai antarmuka native masing-masing model. Standar ini konsisten dengan praktik penelitian evaluasi komparatif dalam MIR \parencite{hadjeres2017, huang2017}.
...
\begin{enumerate}
    \item \textbf{Uji positif:} Koral Bach asli dari korpus music21 (BWV 66.6) dievaluasi dan harus menghasilkan skor tinggi, sesuai ekspektasi bahwa koral Bach merupakan contoh penerapan kaidah Strube yang konsisten.
    \item \textbf{Uji negatif:} Harmonisasi yang sengaja dibuat melanggar kaidah Strube --- dengan kuint sejajar dan resolusi nada penuntun yang salah secara eksplisit --- dievaluasi dan harus menghasilkan skor rendah.
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Pengembangan instrumen:} Penulisan dan pengujian skrip evaluasi Strube berbasis music21.
    \item \textbf{Validasi instrumen:} Pengujian \textit{face validity} menggunakan koral Bach asli dan harmonisasi yang disengaja melanggar.
    \item \textbf{Pengumpulan data:} Generasi 120 sampel output MIDI dari ketiga model di bawah empat kondisi instruksi.
    \item \textbf{Evaluasi otomatis:} Seluruh 120 sampel MIDI dievaluasi menggunakan instrumen yang telah divalidasi; hasil disimpan dalam format CSV terstruktur.
    \item \textbf{Analisis data:} Statistik deskriptif (rata-rata Strube Score per model per kondisi) dan analisis komparatif lintas model dan kondisi.
    \item \textbf{Visualisasi:} Penyajian hasil dalam bentuk grafik batang kelompok (\textit{grouped bar chart}) yang membandingkan ketiga model di bawah empat kondisi.
\end{enumerate}
